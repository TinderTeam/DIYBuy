# Host: fen612.gotoftp4.com  (Version: 5.1.65-community)
# Date: 2014-10-17 21:07:47
...
DROP DATABASE IF EXISTS `fen612`;
CREATE DATABASE `fen612` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `fen612`;
...
INSERT INTO `diy_admin` VALUES (1,'admin','123456','高','market@fuego.cn');
...
CREATE VIEW `diy_group_order_view` AS 
  select `diy_group_order`.`Id` AS `Id`,`diy_group_order`.`productID` AS `productID`,`diy_group_order`.`userID` AS `userID`,`diy_group_order`.`datetime` AS `datetime`,`diy_product`.`name` AS `productName`,`diy_product`.`status` AS `status`,`diy_product`.`total_num` AS `total_num`,`diy_product`.`current_num` AS `current_num`,`diy_product`.`time_end` AS `time_end`,`diy_user`.`name` AS `username` from ((`diy_group_order` join `diy_product` on((`diy_product`.`id` = `diy_group_order`.`productID`))) join `diy_user` on((`diy_user`.`id` = `diy_group_order`.`userID`)));
...
CREATE VIEW `diy_order_view` AS 
  select `diy_order`.`Id` AS `Id`,`diy_order`.`productID` AS `productID`,`diy_order`.`productName` AS `productName`,`diy_order`.`status` AS `orderStatus`,`diy_order`.`totalPrices` AS `totalPrices`,`diy_order`.`user` AS `userName`,`diy_order`.`code` AS `code`,`diy_product`.`status` AS `productStatus` from (`diy_order` join `diy_product` on((`diy_product`.`id` = `diy_order`.`productID`)));
